...
class TheBalanceProvider {
    ...
    fun unknownAddressHasNullBalance() {
        assertThat(BalanceProvider().getBalanceForAddress(WallethAddress("0x123"), ETH_TOKEN)).isNull()
    }

    @Test
    fun weCanSetABalance() {
        val tested = BalanceProvider()
        tested.setBalance(WallethAddress("0x124"),100L, BigInteger("5"), ETH_TOKEN)

        val returned = tested.getBalanceForAddress(WallethAddress("0x124"), ETH_TOKEN)
    ...
    fun weCanUpdateBalance() {
        val tested = BalanceProvider()
        tested.setBalance(WallethAddress("0x124"),100L, BigInteger("5"), ETH_TOKEN)
        tested.setBalance(WallethAddress("0x124"),101L, BigInteger("6"), ETH_TOKEN)
    ...
    fun oldInfoIsRejected() { // important when data is coming from different sources e.g. ligh-client vs etherscan
        val tested = BalanceProvider()
        tested.setBalance(WallethAddress("0x124"),100L, BigInteger("5"), ETH_TOKEN)
        tested.setBalance(WallethAddress("0x124"),99L, BigInteger("6"), ETH_TOKEN)
